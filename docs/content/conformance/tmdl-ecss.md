...
| Other Information | Go library implementing the TM Space Data Link Protocol. This document audits it against the European profile rather than against CCSDS 132.0-B; the CCSDS statement is [the CCSDS 132.0-B-3 PICS](/conformance/tmdl). |
...
Every numbered clause of the normative sections (clause 5.1 to clause 5.6) appears below,
...
Clause 5.3.2.3c requires the six-bit field to hold *the total secondary header length
...
The same wording is in CCSDS 132.0-B-3 clause 4.1.3.2.2.3, so this was never an
ECSS-only defect.
...
Clause 5.3.4.2a fixes the secondary header at 32 bits when it carries the extended
...
Clause 5.2.7.6 gives two codes to two conditions: `11111111111` (0x7FF) when no
...
Clause 5.3.1c caps the whole secondary header at 64 octets. A 63-value length field
...
Clause 5.6.1b makes the field mandatory when the frame is not Reed-Solomon encoded,
and its NOTE makes it optional inside a code block, which already protects the
frame. Clause 5.6.1c requires the choice to hold across the physical channel.
...
Clause 5.1b caps the frame at 2048 octets and nothing enforced it. It is still
...
- The standard was read in full; every numbered clause under clause 5 was extracted
