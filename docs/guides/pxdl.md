...
- **COP-P**, the retransmission procedure. Sequence numbers are carried; the
  state machine that acts on them is a follow-up, in the shape of `pkg/cop`.
- **Directive and status-report contents** (annex B). Variable-length SPDUs
  encode and decode; this package carries the payload without interpreting it.
- **The MAC and PHY sublayers**, and session establishment.
...
- [PICS proforma](../pics/pxdl-pics.md) — conformance statement for this package
