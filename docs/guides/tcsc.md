...
TC randomizes the same way TM does — XOR with a PN sequence from an 8-bit LFSR — but with a different polynomial, so the two sequences are not interchangeable.

- **Polynomial**: h(x) = x^8 + x^6 + x^4 + x^3 + x^2 + x + 1 (CCSDS 231.0-B-4 §6.2)
- **Initial state**: All 1s (0xFF)
- **First 40 digits**: FF 39 9E 5A 68. The TM sequence of CCSDS 131.0-B-5 §10.4.2 opens FF 48 0E C0 9A instead. Using the TM sequence here makes every CLTU unreadable to conformant equipment, and no round-trip test can tell, because XOR is self-inverse.
