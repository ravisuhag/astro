# ICS PROFORMA FOR ROBUST COMPRESSION OF FIXED-LENGTH HOUSEKEEPING DATA
...
| Other Information | Go library. Both directions are explicit state types holding the mask, the build and the previous vector; neither is safe for concurrent use. Integer and bitwise operations only. Byte-slice in, byte-slice out with an explicit bit length — framing is the caller's, per §2.2. |
...
| P2 | Configuration of initial mask vector, M0 | 3.3.1 | M | 0 .. 2^F−1 | any; nil means all zeros, the default §3.3.1's note suggests |
...
### Table B-1: Input and parameters (§3)
...
### Table B-3: Encoding functions (§5.2)
...
sections are §3, §4 and §5; the requirements list in annex A2.2.1 has five
items and every one is an encoder item. There is no decoder section.

The decompressor here is the encoder run backwards. §2.1 lists what a
...
operation examples in §1.6.1's text. Everything above that level is verified by
...
§2.2 says the standard "does not provide a mechanism for identifying the number
...
Likewise §2.2 disclaims sync markers: a corrupt or foreign vector that happens
...
§3.3.2 makes three flags user-specified at every cycle and says nothing about
...
| Knob | Normative? |
|---|---|
| `Robustness` | Yes — §3.3.2a, 0 to 7 |
| `NewMaskInterval` | No — this package's convenience |
| `SendMaskInterval` | No — beyond the §3.3.2c forcing while t ≤ R_t |
| `UncompressedInterval` | No — beyond the §3.3.2d forcing while t ≤ R_t |
...
§3.3.2's note lists the parameters a decompressor need not be told: M0, R_t and
...
| Limit | Value | Why |
|---|---|---|
| Remembered history | 16 cycles | §5.3.2.2 bounds C_t by min(t,15) − R_t, so nothing older can ever be needed. Not a restriction, just the working set. |
