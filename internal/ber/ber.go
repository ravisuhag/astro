...
// CCSDS cross support standards use.
//
// It serves pkg/sle, whose transfer service protocol data units are BER, and
// pkg/csts, whose specification framework is a different set of ASN.1 modules
// carried the same way. The two standards share no data types, but they share
// the encoding, so this sits under both rather than inside either.
//
// Go's encoding/asn1 cannot do this job. It implements DER and rejects the
// context-specific CHOICE tagging that every module in both families relies
// on, so the alternative to hand-rolling was a third-party dependency this
// repository does not take.
//
// What is supported: the universal types these standards use (BOOLEAN,
// INTEGER, NULL, OCTET STRING, OBJECT IDENTIFIER, VisibleString, SEQUENCE,
// SEQUENCE OF), context-specific tags in both primitive and constructed form,
// multi-octet tag numbers, and the definite-length form in both short and long
// variants.
//
...
func AppendInteger(dst []byte, v int64) []byte {
	return AppendElement(dst, ClassUniversal, false, uint32(TagInteger), IntegerContent(v))
}
...
func AppendTaggedInteger(dst []byte, tag uint32, v int64) []byte {
	return AppendElement(dst, ClassContext, false, tag, IntegerContent(v))
}

// encodeIntegerContent produces the minimal two's complement content octets.
// IntegerContent returns the two's-complement content octets of a BER
// INTEGER, without a tag or a length. A caller that is building a tagged
// integer itself needs the content on its own.
func IntegerContent(v int64) []byte {
	if v == 0 {
		return []byte{0}
	}
