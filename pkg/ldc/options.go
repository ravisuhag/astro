...
//
// Clause 3.3.1: the top n-k bits of each sample become an FS codeword, and the low k
...
//
// Clause 3.3.3 fixes the order: every FS codeword first, then every sample's k low
...
// Clause 3.4.1 and hands each one to visit:
//
...
// fit. Clause 3.4.2 notes the option "is only designed to be a useful option when
...
// identifier and any reference sample. Clause 3.6.1: the block goes out unaltered.
...
const (
	...
	maxZeroRun = 63
	// zeroBlockSegment is how many blocks a segment holds, per clause 3.5.2.
	zeroBlockSegment = 64
)
...
//
// Clause 3.5.1: when the block carries a reference sample, only the J-1 samples
// after it are considered, "regardless of whether the reference sample itself
// is zero" (clause 3.7.2). The caller passes the block already trimmed of its
