...
func FuzzDecodeTMTransferFrame(f *testing.F) {
	...
	if frame, err := tmdl.NewTransferFrame(42, 5, []byte("seed-payload"), nil, nil); err == nil {
		if encoded, err := frame.Encode(); err == nil {
			f.Add(encoded)
		}
	}
	if frame, err := tmdl.NewTransferFrame(42, 5, []byte("seed"), nil, []byte{1, 2, 3, 4}); err == nil {
		if encoded, err := frame.Encode(); err == nil {
			f.Add(encoded)
		}
	}
	...
	f.Fuzz(func(t *testing.T, data []byte) {
		...
		_, _ = tmdl.DecodeTransferFrame(data)
	})
}
