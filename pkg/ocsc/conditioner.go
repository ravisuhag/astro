...
//
// The NOTE under §3.2 says encoding may be performed in a streaming fashion:
...
// does exactly that. Each Push marks the frames it is given (§3.3), appends
// them to the carried bit stream, and returns every information block that is
// now complete, conditioned (§3.5-§3.7). Bits short of a full block stay
// buffered for the next Push — no fill is inserted mid-stream, because
// §3.4.2.1.1 permits zero fill only at transmission closure.
//
...
// Close declares transmission closure (§3.4.2.1.1): the buffered remainder,
...
// condition runs one k-digit information block through §3.5, §3.6 and §3.7.
