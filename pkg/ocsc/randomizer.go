...
// Pseudo-randomizer, per CCSDS 142.0-B-1 clause 3.5.
//
...
//
// The generating polynomial is g(D) = D^8 + D^7 + D^5 + D^3 + 1 (clause 3.5.2.1),
// the register starts at all ones (clause 3.5.3.2), and the sequence repeats every
// 255 digits (clause 3.5.3.1).
//
// This is the same sequence pkg/tmsc uses for TM randomization, clause 3.5.2.1's
...
// generatePN runs the LFSR of clause 3.5.2.1 for one full period.
//
// The register is initialized to all ones (clause 3.5.3.2), the output is its top
...
// publishes. Clause 3.5.2.1's note gives the first 40 digits, and
...
//
// Clause 3.5.3.1: the sequence begins at the first digit of each block, so every
