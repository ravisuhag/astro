...
// Mode is the Security Association service type of CCSDS 355.0-B-2 clause 4.2.2.4.
...
// this: Clause 4.2.2.4 defines the three modes, while the annexes name a different
// algorithm per link. Clause E1 gives GMAC for TM and clause E3/clause E4 for AOS and USLP;
// Clause E2 gives AES-CMAC for telecommand.
...
// MAC widths this package accepts, in octets. The clause E1 and clause E2 baselines both
// use 16. Clause 4.2.3.4 f) allows truncating a MAC by dropping its least
...
// GCM tags below 12 octets. CMAC has no such constraint, SP 800-38B clause 6.4
...
// Channel or MAP (clause 4.2.2). Both ends configure a matching SA before the link
...
type SecurityAssociation struct {
	...
	// (clause 4.2.3.4 a), used when the SA carries an explicit Sequence Number field.
	seqCounter []byte
	...
	// only after a MAC verifies (clause 4.2.4.4).
	lastSeq []byte
	seqSeen bool
}

// usesIVAsSequence reports whether the IV doubles as the anti-replay counter.
// Clause 4.1.1.4.2 allows this for counter-mode algorithms, and the clause E1 baseline
...
func (sa *SecurityAssociation) Validate() error {
	// Clause 4.1.1.2.3 reserves both extremes of the SPI range.
	...
	if sa.needsMAC() {
		if sa.usesCMAC() {
			// Clause E2.2 note: CMAC performs no encryption and needs no
			...
			// apply to CMAC: SP 800-38B clause 6.4 permits any truncation, so
...
// ErrIVExhausted instead (clause E1.1 b, and the GCM requirement behind it).
...
// checkAndAdvanceSequence applies the anti-replay rules of clause 2.3.2.3.
//
...
// window (clause 4.2.4.4).
//
// On the reading of clause 2.3.2.3.2 and clause 2.3.2.3.3 taken here: the standard says a
...
func (sa *SecurityAssociation) checkAndAdvanceSequence(received []byte) error {
	...
	if sa.seqSeen {
		...
		// Clause 2.3.2.3.2: accept only a value higher than the stored one.
		if got.Cmp(last) <= 0 {
			return ErrReplayDetected
		}

		// Clause 2.3.2.3.3: and no more than the window ahead of it.
