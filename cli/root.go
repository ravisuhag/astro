package cli

import (
	"embed"

	"github.com/raystack/salt/cli/commander"
	"github.com/spf13/cobra"
)

// New creates the root command for the astro CLI.
func New(docsFS embed.FS) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "astro <command> <subcommand> [flags]",
		Short: "CCSDS and ECSS space communication toolkit",
		Long:  "Astro is a CLI toolkit for working with CCSDS and ECSS space communication protocols.",
		Annotations: map[string]string{
			"help:learn":    "Use 'astro <command> --help' for more information about a command.",
			"help:feedback": "Open an issue at https://github.com/ravisuhag/astro/issues",
		},
		SilenceUsage:  true,
		SilenceErrors: true,
		Version:       buildInfo().String(),
	}

	cmd.AddCommand(sppCmd())
	cmd.AddCommand(eppCmd())
	cmd.AddCommand(timeCmd())
	cmd.AddCommand(tmCmd())
	cmd.AddCommand(tcCmd())
	cmd.AddCommand(caduCmd())
	cmd.AddCommand(cltuCmd())
	cmd.AddCommand(usdlCmd())
	cmd.AddCommand(aosCmd())
	cmd.AddCommand(xtceCmd())
	cmd.AddCommand(pusCmd())
	cmd.AddCommand(ldcCmd())
	cmd.AddCommand(cfdpCmd())
	cmd.AddCommand(ltpCmd())
	cmd.AddCommand(bpCmd())
	cmd.AddCommand(sleCmd())
	cmd.AddCommand(rhcCmd())
	cmd.AddCommand(copCmd())
	cmd.AddCommand(pxdlCmd())
	cmd.AddCommand(pxscCmd())
	cmd.AddCommand(ocscCmd())
	cmd.AddCommand(sdlsCmd())
	cmd.AddCommand(manualCmd(docsFS))
	cmd.AddCommand(versionCmd())

	mgr := commander.New(cmd)
	mgr.Init()

	return cmd
}
